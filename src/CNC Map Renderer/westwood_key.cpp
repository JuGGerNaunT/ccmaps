...
void move_key_to_big(bignum n, int8_t *key, uint32_t klen, uint32_t blen)
{
    uint32_t sign;
    int i;
    ...
    for (i = blen*4; (uint32_t)i > klen; i--)
        ((int8_t *)n)[i-1] = (int8_t)sign;
    for (; i > 0; i--)
        ((int8_t *)n)[i-1] = (int8_t)key[klen-i];
}

void key_to_bignum(bignum n, int8_t *key, uint32_t len)
{
    uint32_t keylen;
    int i;
...
void init_pubkey()
{
    ...
    while (i < strlen(pubkey_str)) {
        ...
        tmp <<= 6;
        tmp |= char2num[(uint8_t)pubkey_str[i++]];
        keytmp[i2++] = (int8_t)((tmp >> 16) & 0xff);
        keytmp[i2++] = (int8_t)((tmp >> 8) & 0xff);
        keytmp[i2++] = (int8_t)(tmp & 0xff);
    }
...
int32_t cmp_bignum(bignum n1, bignum n2, uint32_t len)
{
    n1 += len-1;
    n2 += len-1;
    while (len > 0) {
...
void calc_a_bignum(bignum n1, bignum n2, bignum n3, uint32_t len)
{
    ...
    if (g2_len_x2 >= glob1_len_x2) {
        ...
        for (; len_diff != 0; len_diff--) {
            edi--;
            tmp = (uint16_t)get_mulword((uint32_t *)edi);
            esi--;
            if (tmp > 0) {
...
void decode_westwood_key(const uint8_t* src, uint8_t* dest) {
